...
targets:
  - '*'
description: 'Router: person entities — defer to Logseq/Entity/person and logseq-entity skill'
...
- **Workflow:** Use the **logseq-entity** skill: read `[[Logseq/Entity]]` and `[[Logseq/Entity/person]]`, dedupe before creating, then create or update pages per that SOP; use `.rulesync/config/logseq-entity.md` only when graph pages are missing or insufficient (shared fallback text, not per-type sections).
