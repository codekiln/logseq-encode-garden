...
targets:
  - '*'
description: 'External entity pages should use one canonical URL in the H1 and avoid duplicate links or invented macros'
globs: ['pages/*.md']
---
# Logseq External Entity Pages

Many pages in this graph act as local proxy pages for internet pages, internet entities, projects, posts, documentation, and other external resources.
...
Do not invent Logseq macros. Only add a `{{...}}` macro when the macro name is already used in this graph or the user explicitly requests it.

Known examples already present in this graph include `{{embed ...}}`, `{{video ...}}`, and `{{youtube-timestamp ...}}`.

Do not add `{{iframe ...}}`; it is not a valid established Logseq macro in this graph and it duplicates a normal URL link.
