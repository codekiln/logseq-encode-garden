...
A checklist for exceptional tasks when converting standard markdown (particularly from ChatGPT or similar sources) to LFM. For standard LFM rules, see `logseq-core` / the `logseq-lfm` skill.
...
- [ ] Reference: See `Logseq___How To___Create a Markdown Footnote.md` for details
- [ ] Example conversion:
  ```
...
## Related
- `logseq-core` / `logseq-lfm` skill — standard LFM rules
- `logseq-table-formatter` skill — align tables you keep
