...
* **Journal**: Record the filing in **`journals/YYYY_MM_DD.md`**. Put **new** question pages under **`[[Filed]]`** as a link-only line: `- [[Namespace/Q/Question text]]`. Follow **`[[Logseq/Journal]]`** for the format (mutual exclusivity, “no boilerplate”) and skill **logseq-entity** → [entity-session-journal.md](../../logseq-entity/references/entity-session-journal.md).
...
* In order, read and apply:
  ...
  * `[[Logseq/Entity/Question]]` (`pages/Logseq___Entity___question.md`) — the sole format authority for question pages; follow it completely
...
* **If a matching or near-duplicate question page exists (existing):**
  ...
  * Add a **narrative** journal note **outside** the **`[[Filed]]` / `[[Updated]]`** link lists when those lists are reserved for **`pages/`** graph edits only; link `- [[Namespace/Q/Existing question text]]`. **Unless** you also **edited** that question page—then log that page under **`[[Updated]]`** per **`[[Logseq/Journal]]`**.
...
* Create the page following `[[Logseq/Entity/Question]]` loaded in Step 0.
* **LFM mechanics**: rule **logseq-core** — bullets, tab indentation, no blank lines between bullets (advanced detail: skill **logseq-lfm**).
* **Non-goal:** Do not bulk-edit older `___Q___` pages unless the user explicitly asks.

### Step 4: Add today's journal entry (Filed/Updated + narrative)

* Open today's journal file: `journals/YYYY_MM_DD.md`.
* **New question page (after Step 3):** Under today’s **`[[Filed]]`** list, append a **link-only** line: `- [[Namespace/Q/Question text]]` (forward slashes in the link). That page is **new in `pages/`** today—this satisfies rule **`logseq-core`** (journal updates) without duplicating the same link under **`[[Updated]]`**.
* **Optional narrative:** If your journal style uses a topical block (questions, friction, etc.), you may add a short child or sibling bullet for context—keep it to one sentence max per **`[[Logseq/Journal]]`**.
...
* If the graph defines no **`[[Filed]]`** / **`[[Updated]]`** convention, fall back to a single topical bullet with `- [[Namespace/Q/Question text]]` and still record the new page per **`[[Logseq/Journal]]`** / rule **`logseq-core`**.
...
* Write the answer following `[[Logseq/Entity/Question]]` loaded in Step 0.
* If the answer cannot be determined or requires user input, note it in the appropriate section and inform the user.
...
* **If new page created**: Report the new page path and link, confirm **`[[Filed]]`** placement (or fallback journal) per Step 4, and summarize the answer (or note if the question remains unanswered).
