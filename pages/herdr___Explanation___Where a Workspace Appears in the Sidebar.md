...
- # Where a workspace appears in the sidebar
	...
	- ## What a change would have to touch
		...
		- The grouping pass itself is a small edit; `workspace_list_entries_inner` would bucket on the override and fall back to the membership key. The cost is everywhere else. Roughly forty call sites read `worktree_space()`, and they split into two kinds that a single field would conflate: rendering (row layout, collapse state at `src/ui/sidebar.rs:260-280`, child labels at `:284-301`, the mobile switcher) and behaviour (worktree source resolution, remove, focus-parent-after-remove, drag, the group close and its confirmation, and the `worktree` block of the workspace API at `src/app/creation.rs:507-515`).
