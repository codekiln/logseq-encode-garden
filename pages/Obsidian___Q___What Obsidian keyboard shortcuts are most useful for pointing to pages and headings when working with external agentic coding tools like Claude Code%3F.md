		...
		- [[AI/Response]]
			...
			- ### For headings (needs a community plugin)
				- Obsidian has **no native** "copy link to heading" command. Two widely-used plugins fill this gap:
					- **Easy Copy** (`easy-copy` in Community Plugins) — place the cursor on a heading line, then trigger **Easy Copy: Contextual Copy** (you assign the hotkey). It copies `[[Note#Heading]]` to the clipboard. Works for blocks and paragraphs too.
					- **Copy Link** (GitHub: `greetclammy/copy-link`) — adds commands for copying wikilinks to files, headings, and blocks, including anchor variants like `[[Note#Heading]]` and block URLs.
