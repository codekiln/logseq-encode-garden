...
- # Expand embeds before evaluating queries in the publisher
	...
	- ## One inclusion index serves the publisher and the diff
		- Codex asks the build to record which pages include text from other pages, so that editing an included block rebuilds the pages that show it. The same index answers Codex's `garden impact` when it asks which pages a block edit reaches, and it answers the merge driver later when it asks whether two edits touched the same block. Build the inclusion index once in garden-core and let all three read it.
