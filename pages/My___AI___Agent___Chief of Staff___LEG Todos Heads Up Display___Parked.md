current-as-of:: 2026-08-26T12:06:39-04:00 ET
...
- # LEG Parked
	...
	- ## A memory this repository publishes has been unreadable since the account name changed
		- Two files are tracked under `.claude/projects/`, in a directory named for a home path whose account segment is a name this machine stopped using. They were committed in [e7e03609 add skillshare entity and rulesync skills notes](https://github.com/codekiln/logseq-encode-garden/commit/e7e03609). The harness reads the directory named for the *current* home path, and that one is empty here, so the guidance in those two files has not been loaded by any agent since the account was renamed. The advice is worth keeping — fetch a GitHub repository's details with `gh repo view <owner/repo> --json name,description,homepageUrl,primaryLanguage,stargazerCount,repositoryTopics` rather than spawning an agent to web-fetch it. Hayward recommends moving it into `.rulesync/` where agents read it and deleting the orphaned directory, which is what [[My/Dotfiles]] asks for as well: rulesync configuration rather than memories saved outside a repository. Moving it into `.rulesync/` means running `rulesync generate`, which on 2026-08-26 rewrote only the outputs of the sources that changed. Measured at source 2026-08-26.
			- ~~~sh
			  cd ~/ghq/github.com/codekiln/logseq-encode-garden && git ls-files '.claude/projects/*' && ls ~/.claude/projects/-Users-$(whoami)-ghq-github-com-codekiln-logseq-encode-garden/memory/
			  ~~~
		- **The directory name publishes that retired account name, and the guard cannot see it.** Writing the name into this page's prose was blocked by the identity guard on the first attempt, which is how the exposure came to light; the same name has sat in the tree since `e7e03609` untouched, because the guard scans the contents of staged files and a path that nothing stages again is never handed to it. `git check-ignore` reports the files are not ignored, so this is a gap in what a content scanner can reach rather than an ignore rule hiding them. A directory name is not prose, so a reader looking for exposure in page text never sees it.
