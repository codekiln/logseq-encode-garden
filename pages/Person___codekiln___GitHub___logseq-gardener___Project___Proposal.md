...
- # The logseq-gardener charter
	...
	- ## Pick the parser and the language from tests on real gardens
		- The choice of parser settles the language, the license, and how much of the engine already exists, so these tests come before any other code. Each test runs on the encode garden, on the public `logseq/docs` graph, and on Logseq's own small test graphs.
		- ~~~text
	...
	- ## Ship the commands that agents in this repository run
		...
		- ~~~text
		  ...
		  ~~~
		- A command that decides whether a name already exists finishes the search or returns `Incomplete`, while completion may answer from a partial index. The agent commands in this repository that draft garden pages and check their links switch to these commands as soon as the fixtures pass, so every agent session runs the engine against the real corpus and reports what it gets wrong.
		...
		- Sharing parsed text between worktrees waits for a measurement showing that a worktree's first command is slow.
