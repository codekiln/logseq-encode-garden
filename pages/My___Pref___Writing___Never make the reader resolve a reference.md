see-also:: [[My/Pref/Writing/Be specific and explicit]]

- Repeat the thing you are naming instead of pointing at it with a short reference. Resolving a pointer is work a reader has to do and a writer could have done once: they must hold the antecedent in their head, find it again, and match it, and a reader who loses the match gets nothing from the sentence.
- This is why an AI agent breaks the rule so readily. A model resolves a reference across a page without effort and cannot feel the cost of asking, so the prose it produces is full of pointers that read as economical and land as work.
