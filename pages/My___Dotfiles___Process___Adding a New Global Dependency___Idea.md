...
- # Refining the Process of Adding Dependencies to my Dotfiles with Agent Skills
	- ## Current vision
		...
		- ### Try-before-commit window (Homebrew case)
			- Homebrew is the one dotfiles backend where installing and declaring are decoupled: `brew install x` works whether or not `x` is in the Brewfile. That gap *is* the low-friction "try it out" move this Idea already asks for — install ad hoc, live with it a few days, then decide.
			- The corresponding rule is [[My/Principle/Declarative over Imperative]] applied to Homebrew: `chezmoi/Brewfile` is meant to be the exhaustive record of everything brew manages, not a hand-picked subset. A brew install absent from the Brewfile is legitimate while under evaluation, illegitimate as a permanent state.
			- Garden-facing ritual: once a week or so, diff what's actually brew-installed (formulae marked `--installed-on-request`, plus casks) against the Brewfile, and uninstall anything not explicitly committed. This is the outer loop's weekly heartbeat for Homebrew specifically — same "surface the gap, close it" mechanism as the rest of this Idea, on a fixed cadence instead of lazily.
			- Mirrors [[My/Dotfiles/openspec/specs/brewfile-style]] (only what mise can't do belongs in the Brewfile) with the missing other half: what *does* belong there must actually be there.
			- See [[2026-07-17 Fri]].
	- ## Deprecated
