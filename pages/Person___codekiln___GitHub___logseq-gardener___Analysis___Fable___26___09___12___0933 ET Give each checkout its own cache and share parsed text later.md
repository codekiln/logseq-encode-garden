...
- # Give each checkout its own cache and share parsed text later
	...
	- ## Each checkout gets its own cache directory
		...
		- Codex is right that the configuration belongs in the key. `garden` reads `:file/name-format` and `:journal/page-title-format` out of `logseq/config.edn` to turn each filename into a page name, so it hashes that file along with the checkout path and reparses the graph when the hash changes.
