...
- # Fleet
	...
	- ## The five levels
		- **[[tmux/session]] — one seat.** Named `<seat>-<date>-<repo>`: `hayward-2026-08-24-LEG`, `seneschal-2026-08-24-MK`. The seat name says which role, the date says which day's run, and the repository suffix says which checkout — so `C-b w` answers what a seat is working on without opening it. `LEG` is this garden, `MK` the work knowledge vault, `GL` the langserve repo, `DF` [[My/Dotfiles]]. Both the seat name and the date survive a respawned pane. Sessions are what [[tmux/Option/status-left]] draws, which is why [[tmux/Option/status-left-length]] matters here.
			- **No dots in a session name.** A `-t` target is parsed as `session:window.pane`, so a dot anywhere in the name is read as a separator and every bare target fails against a window or pane that does not exist — `has-session` included, which is what a script would use to check. `DF` rather than `.F` for the dotfiles suffix for this reason: [[tmux/Q/Why does a session name containing a dot break every bare -t target?]].
			- Read the live name with `tmux display-message -p '#{session_name}'`, which names the session the running pane sits in. A name captured at session start ages silently through a rename, so a stored one is a reading rather than a fact.
