...
- # What are some good options for managing [[yazi]] plugins using [[chezmoi]]?
	- ## [[AI Answer]]
		- [[AI/Response]]
			- **Short answer:** The dominant pattern in highly-starred dotfiles repos is a [[chezmoi/Script/run_onchange_]] chezmoi script that embeds a `sha256sum` of `package.toml` so chezmoi re-runs `ya pkg install` automatically whenever the plugin list changes.
			- **Option 1 — [[chezmoi/Script/run_onchange_]] + `package.toml` hash (most idiomatic):**
			...
			- **Option 2 — [[chezmoi/Script/run_once_]] with explicit plugin array (joelazar, 165 ★, highest-starred chezmoi+yazi repo):**
			...
			- **Option 4 — `mise` as intermediate layer (TibiIius/dotfiles):**
				...
				- A [[chezmoi]] [[chezmoi/Script/run_onchange_]] script delegates to `mise run yazi:install`, which calls `ya pkg install`.
				- Useful when you already manage tool versions with [[mise]].
			- **Option 5 — [[chezmoi/Script/run_onchange_]] with `before_` timing for full install including yazi (ulises-jeremias/dotfiles, 126 ★):**
			...
			- **Recommendation:** Option 1 ([[chezmoi/Script/run_onchange_]] + `package.toml` sha256sum) is the most idiomatic chezmoi approach — declarative, automatically re-triggered, and `ya pkg install` is idempotent. Pair it with pinned `rev`/`hash` in `package.toml` for reproducibility.
