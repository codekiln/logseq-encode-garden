...
- # Chief of Staff
	...
	- Every chief is named `<workspace>-chief-of-staff`. That convention is enough to find them and not enough to reach them: an agent can rename itself, so a name recorded earlier may now belong to a different session, and sending to a bare name reaches whichever one currently answers to it. Address by name and the listing's own reference together, and re-read the listing immediately before sending rather than from memory. No listing is a complete roster of them either. `herdr agent list` sees only chiefs occupying a pane, and a chief running as a background session has no herdr agent name at all — at one count it showed three of the five that were live. The cross-session registry sees those, and lists by display name, which is not unique: two sessions of one chief appear as two identical rows and collapse into one unless the row's ref is kept. So absence from either listing means unknown, never that no chief is there.
