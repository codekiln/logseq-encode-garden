...
Use when a user asks to create a first-class flashcard/card page, factor cards out of an existing page, or work with the `/Card/` namespace convention.
...
- Page title shape: `[[Source/Page/Card/Short Card Title]]`.
- File shape: `pages/Source___Page___Card___Short Card Title.md`.
- The `Card` segment denotes a single card entity. Do not create collection pages under `/Card/` unless the user explicitly asks.
- Dedup before creating: exact `/Card/` title, normalized prompt text in `pages/*___Card___*.md`, then matching legacy inline card blocks.
...
2. Choose the target page as `[[Source/Page/Card/Short Card Title]]`.
...
~~~markdown
- {{embed [[Source/Page/Card/Short Card Title]]}}
~~~
...
Source page `[[mise/Architecture]]` with an inline card under `## System Overview` factors to `[[mise/Architecture/Card/Jobs to be Done]]`; the original block becomes `{{embed [[mise/Architecture/Card/Jobs to be Done]]}}`.
